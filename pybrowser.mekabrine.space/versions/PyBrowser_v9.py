...
class CustomBrowser(QMainWindow):
    ...
    def __init__(self):
        ...
        self.cookie_button = QPushButton("🍪")
        self.cookie_button.setFixedSize(30, 30)
        self.cookie_button.clicked.connect(self.load_cookie_for_site)
        self.cookie_button.hide()  # hidden unless cookies exist
        ...
        self.nav_bar_layout.addWidget(self.cookie_button)
    ...
    def load_cookie_for_site(self):
        browser = self.tabs.currentWidget().layout().itemAt(0).widget()
        ...
        if os.path.exists(cookie_file):
            ...
            reply = QMessageBox.question(
                self, "Load Cookies?",
                f"Do you want to load cookies for {domain}?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            if reply == QMessageBox.StandardButton.Yes:
                store = self.profile.cookieStore()
                for c in cookies:
                    cookie_str = f"{c['name']}={c['value']}; domain={c['domain']}; path={c['path']}"
                    if c.get("secure"):
                        cookie_str += "; Secure"
                    if c.get("httponly"):
                        cookie_str += "; HttpOnly"
                    store.setCookie(cookie_str.encode("utf-8"), browser.url())
                QMessageBox.information(self, "Cookies Loaded", f"Cookies for {domain} loaded.")
    ...
    def add_new_tab(self, url=None):
        ...
        browser.urlChanged.connect(self.check_cookie_button)
    ...
    def check_cookie_button(self, url):
        domain = url.host()
        cookie_file = self.cookie_file_for_domain(domain)
        if os.path.exists(cookie_file):
            self.cookie_button.show()
        else:
            self.cookie_button.hide()
