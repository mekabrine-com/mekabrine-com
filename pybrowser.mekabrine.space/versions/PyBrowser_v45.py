...
class AIAssistantWindow(QMainWindow):
    ...
    def start_agent_mode(self, user_request: str):
        ...
        browser_window = self.get_most_recent_browser()
        if not browser_window:
            self.add_message_bubble("Error: No browser window found. Please open a browser window first.", "assistant")
            return

        self.add_message_bubble("🤖 Activating agent mode...", "status")
...
class CustomBrowser(QMainWindow):
    ...
    def navigate_to_url(self, url: str):
        QTimer.singleShot(0, lambda: self._navigate_to_url_impl(url))

    def _navigate_to_url_impl(self, url: str):
        b = self.get_current_browser()
        if b:
            b.setUrl(QUrl(url))

    def search_for(self, query: str):
        QTimer.singleShot(0, lambda: self._search_for_impl(query))

    def _search_for_impl(self, query: str):
    ...
    def type_text(self, text: str):
        QTimer.singleShot(0, lambda: self._type_text_impl(text))
    ...
    def scroll_page(self, direction: str):
        QTimer.singleShot(0, lambda: self._scroll_page_impl(direction))

    def _scroll_page_impl(self, direction: str):
        b = self.get_current_browser()
        if b:
    ...
    def click_element(self, description: str):
        QTimer.singleShot(0, lambda: self._click_element_impl(description))

    def _click_element_impl(self, description: str):
        b = self.get_current_browser()
        if b:
