...
from PyQt6.QtGui import (
    QIcon, QDrag, QColor, QCursor, QPainter
)
...
DEFAULT_SETTINGS = {
    ...
    "ai_base_url": "https://api.voidai.app/v1/chat/completions",
    "ai_api_key": "",
...
class SettingsDialog(QDialog):
    def __init__(self, settings: dict, parent=None):
        ...
        self.home_mode = settings.get("home_mode", "engine")
        self.home_url = settings.get("home_custom_url", "")

        self.ai_base_url = settings.get("ai_base_url", "https://api.voidai.app/v1/chat/completions")
        self.ai_api_key = settings.get("ai_api_key", "")
        self.ai_model = settings.get("ai_model", "gpt-4.1-mini")
        self.ai_max_tokens = settings.get("ai_max_tokens", 500)
        ...
        layout.addWidget(self.home_mode_combo)

        self.home_url_edit = QLineEdit(self)
        self.home_url_edit.setPlaceholderText("https://www.example.com/")
        layout.addWidget(self.home_url_edit)

        def init_home():
            if self.home_mode == "custom":
                self.home_mode_combo.setCurrentIndex(1)
                self.home_url_edit.setText(self.home_url)
        ...
        # Browser secondary/text color
        self.text_label = QLabel(f"Browser secondary (text) color: {self.text_color}")
        ...
        # AI settings
        layout.addWidget(QLabel("AI Assistant base URL:"))
        self.ai_base_url_edit = QLineEdit(self.ai_base_url, self)
        layout.addWidget(self.ai_base_url_edit)

        layout.addWidget(QLabel("AI Assistant API key:"))
        self.ai_api_key_edit = QLineEdit(self.ai_api_key, self)
        self.ai_api_key_edit.setEchoMode(QLineEdit.EchoMode.Password)
        layout.addWidget(self.ai_api_key_edit)

        layout.addWidget(QLabel("AI Assistant model:"))
        self.ai_model_edit = QLineEdit(self.ai_model, self)
        layout.addWidget(self.ai_model_edit)

        layout.addWidget(QLabel("AI Assistant max tokens:"))
        self.ai_max_tokens_edit = QLineEdit(str(self.ai_max_tokens), self)
        layout.addWidget(self.ai_max_tokens_edit)
    ...
    def pick_text_color(self):
        col = QColorDialog.getColor(QColor(self.text_color), self, "Select text color")
        if col.isValid():
            self.text_color = col.name()
            self.text_label.setText(f"Browser secondary (text) color: {self.text_color}")
    ...
    def get_values(self) -> dict:
        ...
        home_url = self.home_url_edit.text().strip() if mode == "custom" else ""
        try:
            max_toks = int(self.ai_max_tokens_edit.text().strip() or "500")
        except ValueError:
            max_toks = 500
        return {
            ...
            "home_mode": mode,
            "home_custom_url": home_url,
            "ai_base_url": self.ai_base_url_edit.text().strip() or "https://api.voidai.app/v1/chat/completions",
            "ai_api_key": self.ai_api_key_edit.text(),
            "ai_model": self.ai_model_edit.text().strip() or "gpt-4.1-mini",
            "ai_max_tokens": max_toks,
        }
...
class ModificationsDialog(QDialog):
    def __init__(self, settings: dict, parent=None):
        ...
        self.view_combo.addItems(["None", "Black and white", "Invert", "Increased contrast", "Cracked glass"])
        style_code = settings.get("view_style", "none")
        index_map = {"none": 0, "bw": 1, "invert": 2, "contrast": 3, "cracked": 4}
    ...
    def get_values(self) -> dict:
        ...
        code = {0: "none", 1: "bw", 2: "invert", 3: "contrast", 4: "cracked"}.get(idx, "none")
        return {
...
class AIAgentWorker(QThread):
    ...
    action_taken = pyqtSignal(str)
    finished_signal = pyqtSignal()
    error_signal = pyqtSignal(str)
    execute_gui_action = pyqtSignal(str, str)  # (action_type, parameter)

    def __init__(self, user_request: str, settings: dict, browser_window):
        super().__init__()
    ...
    def run(self):
        try:
            self.status_update.emit("Agent mode activated...")
            
            # Build system prompt for agent
            system_prompt = """You are a browser automation assistant. You control a web browser to help users accomplish tasks.

Available actions:
1. navigate <url> - Navigate to a URL
2. search <query> - Search for something using the default search engine
3. click <element_description> - Click on an element (you'll need to describe it)
4. type <text> - Type text into the currently focused input
5. scroll <direction> - Scroll up or down
6. back - Go back in history
7. forward - Go forward in history
8. refresh - Refresh the page
9. new_tab <url> - Open a new tab with optional URL
10. close_tab - Close current tab
11. get_url - Get the current page URL
12. get_title - Get the current page title
13. wait <seconds> - Wait for specified seconds
14. done - Mark task as complete

Respond with ONE action at a time in this format:
ACTION: <action_name> <parameters>
REASON: <brief explanation>

After each action, wait for the result before continuing. Think step by step."""

            messages = [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": f"Please help me with this task: {self.user_request}"},
            ]

            max_steps = 20
            step = 0

            while step < max_steps and not self.should_stop:
                step += 1
                self.status_update.emit(f"Step {step}/{max_steps}...")

                # Call AI API
                response = self.call_ai_api(messages)
                if not response:
                    self.error_signal.emit("Failed to get AI response")
                    break
                ...
                if not action_line:
                    self.status_update.emit("No clear action found, asking AI to clarify...")
                    messages.append({"role": "user", "content": "Please provide a clear ACTION and REASON."})
                    continue

                # Execute action
                result = self.execute_action(action_line)
                self.action_taken.emit(f"{action_line}\n→ {result}")

                if "done" in action_line.lower():
                    self.status_update.emit("Task completed!")
                    break

                messages.append({"role": "user", "content": f"Result: {result}"})
                time.sleep(1)  # Brief delay between actions

            if step >= max_steps:
                self.status_update.emit("Reached maximum steps")
    ...
    def call_ai_api(self, messages):
        try:
            base_url = self.settings.get("ai_base_url", "https://api.voidai.app/v1/chat/completions")
            api_key = self.settings.get("ai_api_key", "").strip()
            model = self.settings.get("ai_model", "gpt-4.1-mini")
            max_tokens = int(self.settings.get("ai_max_tokens", 500) or 500)
            ...
            if choices:
                ...
                if not content and isinstance(choice0.get("delta"), dict):
    ...
    def execute_action(self, action: str):
        ...
        try:
            ...
            if cmd == "navigate":
                ...
                time.sleep(1.5)  # Give time for navigation
                return f"Navigated to {url}"

            elif cmd == "search":
                self.browser_window.search_for(param)
                time.sleep(1.5)
                return f"Searched for: {param}"

            elif cmd == "back":
                self.browser_window.go_back()
                time.sleep(0.5)
                return "Went back"

            elif cmd == "forward":
                self.browser_window.go_forward()
                time.sleep(0.5)
                return "Went forward"

            elif cmd == "refresh":
                self.browser_window.reload_page()
                time.sleep(0.5)
                return "Refreshed page"

            elif cmd == "new_tab":
                url = param if param else None
                self.browser_window.add_new_tab(url)
                time.sleep(1.0)
                return f"Opened new tab" + (f" with {url}" if url else "")

            elif cmd == "close_tab":
                current_idx = self.browser_window.tabs.currentIndex()
                if self.browser_window.tabs.count() > 1:
                    self.browser_window.close_tab(current_idx)
                    time.sleep(0.5)
                    return "Closed tab"
                return "Cannot close last tab"

            elif cmd == "get_url":
                url = self.browser_window.get_current_url()
                return f"Current URL: {url}"

            elif cmd == "get_title":
                title = self.browser_window.get_current_title()
                ...
                self.browser_window.type_text(param)
                time.sleep(0.5)
                return f"Typed: {param}"

            elif cmd == "scroll":
                direction = param.lower()
                self.browser_window.scroll_page(direction)
                time.sleep(0.5)
                return f"Scrolled {direction}"

            elif cmd == "click":
                self.browser_window.click_element(param)
                time.sleep(1.0)
                return f"Attempted to click: {param}"
...
# -------------------------
# AI Assistant window with Agent Mode
# -------------------------
class AIAssistantWindow(QMainWindow):
    def __init__(self, settings: dict, parent=None):
        super().__init__(parent)
        self.setWindowTitle("AI Assistant - PyBrowser")
        self.settings = settings
        self.messages = [
            {"role": "system", "content": "You are PyBrowser's helpful AI assistant. You can chat normally or take control of the browser in agent mode to accomplish tasks."}
        ]
        ...
        # Agent status bar
        self.agent_status_bar = QWidget()
        status_layout = QHBoxLayout(self.agent_status_bar)
        status_layout.setContentsMargins(8, 4, 8, 4)
        self.agent_status_label = QLabel("Agent Mode: Inactive")
        self.agent_status_label.setStyleSheet("font-weight: bold; color: #888;")
        ...
        self.agent_status_bar.setStyleSheet("background-color: #222;")
        outer.addWidget(self.agent_status_bar)
        ...
        self.input_edit.setPlaceholderText("Type a message or ask me to do something...")
        self.input_edit.returnPressed.connect(self.on_send)  # Send on Enter key
    ...
    def add_message_bubble(self, text: str, role: str, is_action: bool = False):
        label = QLabel(text, self)
        label.setWordWrap(True)
        
        if is_action:
            label.setStyleSheet(
                "background-color: #1a472a; color: #aaffaa; padding: 6px; border-radius: 6px; font-family: monospace;"
            )
        elif role == "user":
            label.setStyleSheet(
                "background-color: #2d7dd2; color: white; padding: 6px; border-radius: 6px;"
            )
        elif role == "status":
            label.setStyleSheet(
                "background-color: #555555; color: #ffaa00; padding: 6px; border-radius: 6px; font-style: italic;"
            )
        else:
            label.setStyleSheet(
    ...
    def on_send(self):
        ...
        # Decide whether to use agent mode or normal chat
        agent_keywords = ["navigate", "search for", "go to", "open", "find", "click", "browse", "look up"]
        should_use_agent = any(keyword in text.lower() for keyword in agent_keywords)

        if should_use_agent and not self.agent_worker:
            self.start_agent_mode(text)
        else:
            self.call_api()

    def start_agent_mode(self, user_request: str):
        self.add_message_bubble("🤖 Creating new browser window for agent...", "status")
        
        # Use the same profile so cookies / logins match the main browser
        browser_window = CustomBrowser(profile_name="PyBrowserProfile")
        browser_window.setWindowTitle("PyBrowser - AI Agent Controlled")
        browser_window.show()
        browser_window.raise_()
        browser_window.activateWindow()
        
        # Wait a bit for the window to initialize
        QTimer.singleShot(1000, lambda: self._start_agent_worker(user_request, browser_window))

    def _start_agent_worker(self, user_request: str, browser_window):
        self.add_message_bubble("🤖 Agent mode activated!", "status")
        self.agent_status_label.setText("Agent Mode: Active")
        self.agent_status_label.setStyleSheet("font-weight: bold; color: #00ff00;")
        self.stop_button.setEnabled(True)
        self.input_edit.setEnabled(False)
        self.send_button.setEnabled(False)

        self.agent_worker = AIAgentWorker(user_request, self.settings, browser_window)
        self.agent_worker.status_update.connect(lambda msg: self.add_message_bubble(msg, "status"))
        self.agent_worker.action_taken.connect(lambda msg: self.add_message_bubble(msg, "assistant", is_action=True))
        self.agent_worker.finished_signal.connect(self.on_agent_finished)
        self.agent_worker.error_signal.connect(lambda msg: self.add_message_bubble(f"❌ {msg}", "assistant"))
        self.agent_worker.start()

    def stop_agent(self):
        if self.agent_worker:
            self.agent_worker.stop()
            self.add_message_bubble("Agent stopped by user", "status")
    ...
    def get_most_recent_browser(self):
        from __main__ import CustomBrowser
        if CustomBrowser.INSTANCES:
            return CustomBrowser.INSTANCES[-1]
        return None
    ...
    def call_api(self):
        ...
        self.add_message_bubble(content, "assistant")
...
class CustomBrowser(QMainWindow):
    INSTANCES = []
    _profile_counter = 0  # Counter for unique profiles
    ...
    def scroll_page(self, direction: str):
        b = self.get_current_browser()
        if b:
            amount = 300 if "down" in direction.lower() else -300
    ...
    def click_element(self, description: str):
        b = self.get_current_browser()
        if b:
            # Simple heuristic: try to click elements containing the text
            script = f"""
            (function() {{
                var desc = {json.dumps(description.lower())};
                var elements = document.querySelectorAll('button, a, input[type="submit"], [role="button"]');
                for (var i = 0; i < elements.length; i++) {{
                    var el = elements[i];
                    var text = (el.innerText || el.value || el.title || '').toLowerCase();
    ...
    def load_settings(self):
        ...
        data.setdefault("ai_base_url", "https://api.voidai.app/v1/chat/completions")
        data.setdefault("ai_api_key", "")
        data.setdefault("ai_model", "gpt-4.1-mini")
        data.setdefault("ai_max_tokens", 500)
        return data
        ...
        if style == "none":
            ...
            (function(){
                try{
                    ...
                    if (document.body) document.body.style.filter = 'contrast(1.5)';
                    var ov = document.getElementById('pybrowser-view-style-overlay');
                    if (ov) ov.remove();
                }catch(e){}
            })();
    ...
    # --- Settings / Modifications / Permissions / AI ---
    ...
    def open_ai_assistant_window(self):
        if self.ai_window is None or not self.ai_window.isVisible():
            self.ai_window = AIAssistantWindow(self.settings, self)
            self.ai_window.show()
        else:
    ...
    def open_menu(self):
        ...
        ai_action = menu.addAction("AI Assistant")
        ...
        ai_action.triggered.connect(self.open_ai_assistant_window)
