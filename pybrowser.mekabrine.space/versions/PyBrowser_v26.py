...
class DownloadItemWidget(QWidget):
    def __init__(self, filename: str, parent=None):
        ...
        self.bar.setMaximum(0)  # indeterminate until we know total
    ...
    def set_value(self, value: int):
        if self.bar.maximum() == 0 and value > 0:
            # still indeterminate; keep it until we receive total
            pass
        else:
...
class CustomBrowser(QMainWindow):
    ...
    # Position downloads panel on resize
    ...
    def handle_download(self, download_item):
        ...
        QWebEngineDownloadRequest:
          - setDownloadDirectory(dir), setDownloadFileName(name), accept()
        ...
        def finish():
            w.finish()
            # delay cleanup so user can see "Downloaded ..."
            QTimer.singleShot(3200, self.downloads_panel.cleanup)
            QTimer.singleShot(3200, self.position_downloads_panel)
