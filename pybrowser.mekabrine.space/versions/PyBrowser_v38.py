...
from PyQt6.QtCore import (
    QUrl, Qt, QTimer, QMimeData, QByteArray, QRect
)
...
DEFAULT_SETTINGS = {
    ...
    "bookmarks": [],          # list of {"title": str, "url": str}
}
...
class DragSafeTabBar(QTabBar):
    ...
    def find_browser_window(self):
        ...
        while parent is not None:
            from_main = getattr(parent, "__class__", None)
            if from_main is not None and parent.__class__.__name__ == "CustomBrowser":
                return parent
...
class AdditionsDialog(QDialog):
    def __init__(self, settings: dict, parent=None):
        ...
        # no extra layout; just OK/Cancel
...
class CustomBrowser(QMainWindow):
    def __init__(self):
        ...
        self.tabs.setTabsClosable(False)  # we handle close via hover ❌
    ...
    def setup_nav_bar(self):
        ...
        layout.addWidget(self.bookmark_button)  # star to the left of new tab
    ...
    def setup_bookmarks_bar(self):
        ...
        self.bookmarks_layout.setContentsMargins(5, 0, 5, 0)
        self.bookmarks_layout.setSpacing(4)
        self.bookmarks_bar.setLayout(self.bookmarks_layout)
    ...
    def refresh_bookmarks_bar(self):
        ...
        for bm in self.bookmarks:
            ...
            url = bm.get("url", "")
            btn = BookmarkButton(title, url, self.bookmarks_bar)
            btn.clicked.connect(lambda checked=False, u=url: self.activate_bookmark(u))
    ...
    def on_bookmark_button_clicked(self):
        ...
        def on_accept():
            ...
            if removing:
                ...
                idx = self.is_url_bookmarked(u)
                if idx >= 0:
                    self.bookmarks[idx] = {"title": t, "url": u}
                else:
                    self.bookmarks.append({"title": t, "url": u})
            self.save_settings()
            self.refresh_bookmarks_bar()
            self.update_bookmark_button_state()
            dlg.accept()
    ...
    # --- Settings / Additions / Permissions ---
