...
class SettingsDialog(QDialog):
    def __init__(self, settings: dict, parent=None):
        ...
        self.logins_button.clicked.connect(lambda: self.open_logins_dialog())
        layout.addWidget(self.logins_button)
    ...
    def open_logins_dialog(self):
        dlg = LoginsDialog(self._settings, self)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            vals = dlg.get_values()
            self._settings["master_email"] = vals["master_email"]
            self._settings["master_password"] = vals["master_password"]
...
class ModificationsDialog(QDialog):
    def __init__(self, settings: dict, parent=None):
        ...
        self.view_combo.addItems(["None", "Black and white", "Invert", "Increased contrast", "Dramatic", "Cracked glass"])
        style_code = settings.get("view_style", "none")
        index_map = {"none": 0, "bw": 1, "invert": 2, "contrast": 3, "dramatic": 4, "cracked": 5}
    ...
    def get_values(self) -> dict:
        ...
        code = {0: "none", 1: "bw", 2: "invert", 3: "contrast", 4: "dramatic", 5: "cracked"}.get(idx, "none")
        return {
...
class CustomBrowser(QMainWindow):
    ...
    def apply_view_style(self, browser: QWebEngineView | None = None):
        ...
        if style == "none":
            ...
            (function(){
                try{
                    // Apply cinematic/dramatic filters: contrast, saturation boost, slight blue tint, vignette
                    document.documentElement.style.filter = 'contrast(1.3) saturate(1.2) brightness(0.95) sepia(0.1) hue-rotate(190deg)';
                    if (document.body) document.body.style.filter = 'contrast(1.3) saturate(1.2) brightness(0.95) sepia(0.1) hue-rotate(190deg)';
                    
                    // Remove any existing overlay
                    var existingOv = document.getElementById('pybrowser-view-style-overlay');
                    if (existingOv) existingOv.remove();
                    
                    // Add vignette overlay for dramatic effect
                    ...
                    ov.style.background = 'radial-gradient(ellipse at center, transparent 0%, transparent 40%, rgba(0,10,20,0.3) 100%)';
