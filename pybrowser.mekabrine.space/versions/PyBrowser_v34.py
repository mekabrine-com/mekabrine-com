...
class AdditionsDialog(QDialog):
    def __init__(self, settings: dict, parent=None):
        ...
        self.cb_block_ads = QCheckBox("Block ads")
        self.cb_block_ads.setChecked(settings.get("block_ads", False))
        layout.addWidget(self.cb_block_ads)

        self.cb_kill_ai = QCheckBox("Kill AI")
        self.cb_kill_ai.setChecked(settings.get("kill_ai", False))
        layout.addWidget(self.cb_kill_ai)

        self.cb_im_broke = QCheckBox("I'm broke")
        self.cb_im_broke.setChecked(settings.get("im_broke", False))
        layout.addWidget(self.cb_im_broke)
    ...
    def get_values(self) -> dict:
        return {
            "block_ads": self.cb_block_ads.isChecked(),
            "kill_ai": self.cb_kill_ai.isChecked(),
            "im_broke": self.cb_im_broke.isChecked(),
        }
...
class CustomBrowser(QMainWindow):
    ...
    def detach_tab(self, index: int):
        ...
        new_window.show()
        pos = QCursor.pos()
        new_window.move(pos.x() - new_window.width() // 2,
                        pos.y() - new_window.height() // 2)
    ...
    def apply_page_additions(self, browser: QWebEngineView | None = None):
        ...
        # Kill AI: popup + hide Google AI UI
        ...
        # Block ads: generic + YouTube-specific
        if self.settings.get("block_ads", False):
            script_ads = """
            (function(){
                try{
                    const style = document.createElement('style');
                    style.textContent = `
                        [id*="ad"], [class*="ad"], [class*="Ad"], [id*="Ad"],
                        .ads, .adsbygoogle, .ad-container, .ad-banner,
                        iframe[src*="doubleclick"], iframe[src*="googlesyndication"] {
                            display: none !important;
                        }
                        img[alt*="ad"], img[alt*="Ad"] {
                            display: none !important;
                        }
                    `;
                    ...
                    if (location.hostname.indexOf('youtube.com') !== -1) {
                        ...
                        style2.textContent = `
                            ...
                            .ytp-ad-player-overlay, #player-ads {
                                display: none !important;
                            }
                        `;
            ...
            """
            browser.page().runJavaScript(script_ads)

        # "I'm broke" flag is stored but has no explicit behavior yet
