...
from PyQt6.QtWidgets import (
    ...
    QGraphicsBlurEffect, QScrollArea, QSlider
)
...
class SettingsDialog(QDialog):
    ...
    def show_logins_menu(self):
        ...
        self.stack_layout.addWidget(QLabel("Master Password:"))
        self.password_edit = QLineEdit(self._settings.get("master_password", ""), self)
        self.password_edit.setEchoMode(QLineEdit.EchoMode.Password)
        self.password_edit.setPlaceholderText("Enter master password")
        self.stack_layout.addWidget(self.password_edit)
        
        self.stack_layout.addWidget(QLabel("Confirm Password:"))
        self.confirm_edit = QLineEdit(self)
        self.confirm_edit.setEchoMode(QLineEdit.EchoMode.Password)
        self.confirm_edit.setPlaceholderText("Confirm password")
        self.stack_layout.addWidget(self.confirm_edit)
    ...
    def accept(self):
        ...
        if self.current_page == "logins":
            ...
            self._settings["master_email"] = self.email_edit.text().strip()
            self._settings["master_password"] = password
    ...
    def get_values(self) -> dict:
        ...
        result.update({
            ...
            "page_bg_color": self._settings.get("page_bg_color", "#ffffff"),
            "downloads_dir": self._settings.get("downloads_dir", r"C:\PyBrowser\Downloads"),
            "master_email": self._settings.get("master_email", ""),
            "master_password": self._settings.get("master_password", ""),
        })
                ...
                (function() {{
                    ...
                    for (var i = 0; i < inputs.length; i++) {{
                        if (document.activeElement === inputs[i]) {{
                            inputs[i].value = {json.dumps(master_password)};
