...
class SettingsPopup(QDialog):
    ...
    def pick_color(self, key, btn):
        ...
        if color.isValid():
            ...
            if self.parent.settings.get("custom_colors", False):
...
class ToolsPopup(QDialog):
    ...
    def inject_js(self):
        ...
        if not code:
            return
        b = self.parent.get_current_browser()
        if b:
            # run in the page context, not main window
            b.page().runJavaScript(code)


# -------------------------
# DragSafeTabBar for context menu + safer tab reordering
# -------------------------
...
class DragSafeTabBar(QTabBar):
    ...
    def show_context_menu(self, pos: QPoint):
        ...
        if action == action_refresh:
            parent.parent().refresh_tab(idx)
        elif action == action_close:
            parent.parent().close_tab(idx)
...
class CustomBrowser(QMainWindow):
    ...
    def browser_at(self, tab_index: int):
    ...
    def apply_custom_colors_to_all(self, force: bool = False, reload_if_disabled: bool = False):
        enabled = bool(self.settings.get("custom_colors", False))
    ...
    def add_new_tab(self, url=None):
        ...
        browser.iconChanged.connect(lambda i, b=browser: self.update_tab_icon(b, i))
        ...
        js = f"""
            (function() {{
              try {{
                ...
                console.error("inject_custom_colors error", e);
              }}
            }})();
        """
